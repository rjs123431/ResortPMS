...
import { OrderItemCancelReasonType } from '@/types/pos.types';
import type { OrderItemDto } from '@/types/pos.types';

const REASON_TYPE_LABELS: Record<OrderItemCancelReasonType, string> = {
  [OrderItemCancelReasonType.GuestRequest]: 'Guest request',
  [OrderItemCancelReasonType.WrongOrder]: 'Wrong order',
  [OrderItemCancelReasonType.OutOfStock]: 'Out of stock',
  [OrderItemCancelReasonType.Duplicate]: 'Duplicate',
  [OrderItemCancelReasonType.Other]: 'Other',
};
...
export const RemoveItemDialog = ({
  ...
  const [reasonType, setReasonType] = useState<OrderItemCancelReasonType>(OrderItemCancelReasonType.GuestRequest);
  const [reason, setReason] = useState('');

  useEffect(() => {
    if (open) {
      setReasonType(OrderItemCancelReasonType.GuestRequest);
      setReason('');
    }
  }, [open]);
        ...
        <DialogPanel className="w-full max-w-md rounded-xl bg-white shadow-xl dark:bg-gray-800 pointer-events-auto">
          ...
          <div className="space-y-4 px-5 py-4">
            <div>
              ...
              <select
                value={reasonType}
                onChange={(e) => setReasonType(Number(e.target.value) as OrderItemCancelReasonType)}
              ...
              >
                {(Object.entries(REASON_TYPE_LABELS) as unknown as [OrderItemCancelReasonType, string][]).map(([value, label]) => (
