...
namespace PMS.Notifications
{
    ...
    public class NotificationAppService : PMSAppServiceBase, INotificationAppService
    {
        ...
        private readonly IRepository<GlobalNotification> _globalNotificationRepository;

        public NotificationAppService(
            INotificationDefinitionManager notificationDefinitionManager,
            IUserNotificationManager userNotificationManager,
            INotificationSubscriptionManager notificationSubscriptionManager,
            IRepository<GlobalNotification> globalNotificationRepository)
        {
            _notificationDefinitionManager = notificationDefinitionManager;
            _userNotificationManager = userNotificationManager;
            _notificationSubscriptionManager = notificationSubscriptionManager;
            _globalNotificationRepository = globalNotificationRepository;
        }
        ...
        public async Task<GetNotificationsOutput> GetUserNotifications(GetUserNotificationsInput input)
        {
            ...
            var totalRecords = await _userNotificationManager.GetUserNotificationCountAsync(
                AbpSession.ToUserIdentifier(), null, input.StartDate, input.EndDate
                );
            ...
            return new GetNotificationsOutput(totalCount, unreadCount, totalRecords, notifications);
        }
        ...
        public async Task<ListResultDto<GlobalNotificationDto>> GetGlobalNotifications()
        {
            var currentTime = Clock.Now;
            var notifications = await _globalNotificationRepository.GetAll().AsNoTracking()
                                    .Where(x => currentTime >= x.StartDate && currentTime <= x.EndDate)
                                    .ToListAsync();
            var result = ObjectMapper.Map<List<GlobalNotificationDto>>(notifications);

            return new ListResultDto<GlobalNotificationDto>(result);
        }

        [AbpAuthorize(PermissionNames.Pages_Global_Notifications)]
        public async Task CreateGlobanNotificationAsync(GlobalNotificationDto input)
        {
            var entity = ObjectMapper.Map<GlobalNotification>(input);

            await _globalNotificationRepository.InsertAsync(entity);
        }
    }
}
