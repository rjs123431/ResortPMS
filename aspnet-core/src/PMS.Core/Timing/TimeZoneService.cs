...
namespace PMS.Timing
{
    public class TimeZoneService : ITimeZoneService, ITransientDependency
    {
        ...
        public TimeZoneInfo GetUserTimezone(int tenantId, long userId)
        {
            var userTimeZone = _settingManager.GetSettingValueForUser(TimingSettingNames.TimeZone, tenantId, userId);

            if (tenantId > 0 && string.IsNullOrEmpty(userTimeZone))
            {
                userTimeZone = _settingManager.GetSettingValueForTenant(TimingSettingNames.TimeZone, tenantId);
            }

            if (string.IsNullOrEmpty(userTimeZone))
                userTimeZone = _settingManager.GetSettingValueForApplication(TimingSettingNames.TimeZone);

            return FindTimeZoneById(userTimeZone);
        }
    }
}
