...
import { SearchGuestDialog, SelectedGuest } from './SearchGuestDialog';
import { AddPaymentDialog } from './AddPaymentDialog';
import { AddExtraBedDialog } from './AddExtraBedDialog';
...
export const FindAvailableRoomPage = () => {
  ...
  const [seniorCountsByType, setSeniorCountsByType] = useState<Record<string, number>>({});
  ...
  const reservationDetailLines = useMemo(() => {
    ...
    return selectedSummary.rows.map((row) => {
      const quantity = row.selected;
      const grossAmount = round2(quantity * nights * row.baseRate);
      const maxSeniorCount = Math.max(0, quantity * Math.max(1, row.maxAdults));
      const requestedSeniorCount = seniorCountsByType[row.roomTypeId] ?? 0;
  ...
  }, [searchCriteria, selectedSummary.rows, seniorCountsByType]);
  ...
  const allRoomTypeIds = useMemo(() => (roomTypes ?? []).map((rt) => rt.id), [roomTypes]);

  const isAllRoomTypesSelected =
    allRoomTypeIds.length > 0 && allRoomTypeIds.every((id) => selectedRoomTypeIds.includes(id));
  ...
  const toggleAllRoomTypes = () => {
    setSelectedRoomTypeIds(isAllRoomTypesSelected ? [] : allRoomTypeIds);
  };
  ...
  const handleSeniorCountChange = (roomTypeId: string, value: string, maxValue: number) => {
    const parsed = Number(value);
    const next = Number.isNaN(parsed) ? 0 : Math.max(0, Math.min(Math.floor(parsed), maxValue));
    setSeniorCountsByType((prev) => ({
      ...prev,
      [roomTypeId]: next,
    }));
  };

  const removeSelectedRoomType = (roomTypeId: string) => {
    setSelectedAmounts((prev) => ({
      ...prev,
      [roomTypeId]: 0,
    }));
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const roomEntries = reservationDetailLines.flatMap((line) =>
          ...
          <section className="rounded-lg bg-white p-5 shadow dark:bg-gray-800">
                ...
                <table className="min-w-full border border-gray-200 text-sm dark:border-gray-700">
                  ...
                  <tbody>
                    {reservationDetailLines.map((line) => (
                      <tr key={line.roomTypeId}>
                        <td className="border border-gray-200 p-2 dark:border-gray-700">
                          <div className="flex items-center justify-between gap-2">
                            <span>{line.roomTypeName}</span>
                            <button
                              type="button"
                              onClick={() => removeSelectedRoomType(line.roomTypeId)}
                        ...
                        <td className="border border-gray-200 p-2 text-right dark:border-gray-700">{line.quantity}</td>
                        ...
                        <td className="border border-gray-200 p-2 text-right dark:border-gray-700">
                          <div className="flex items-center justify-end gap-1">
                            <button
                              ...
                              onClick={() =>
                                handleSeniorCountChange(line.roomTypeId, String(line.seniorCount - 1), line.maxSeniorCount)
                              }
                            >
                              -
                            </button>
                            <input
                              ...
                              onChange={(e) => handleSeniorCountChange(line.roomTypeId, e.target.value, line.maxSeniorCount)}
                              className="w-20 rounded border border-gray-300 p-1 text-right text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
                            />
                            <button
                              type="button"
            ...
            <div className="mt-4 rounded border border-gray-200 bg-slate-50 p-3 dark:border-gray-700 dark:bg-gray-700/30">
              ...
              <div className="mt-2 space-y-1">
                {reservationDetailLines.map((line) => (
                  <p key={`bottom-summary-${line.roomTypeId}`} className="text-sm text-gray-600 dark:text-gray-300">
                    {line.quantity} {line.roomTypeName} room{line.quantity > 1 ? 's' : ''} x {line.nights} night{line.nights > 1 ? 's' : ''}{line.seniorCount > 0 ? ` (${line.seniorCount} senior${line.seniorCount === 1 ? '' : 's'})` : ''} = {formatMoney(line.netAmount)}
                  </p>
                ))}
          ...
          <section className="rounded-lg bg-white p-5 shadow dark:bg-gray-800">
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              ...
              <div className="rounded border border-gray-200 bg-slate-50 p-3 dark:border-gray-700 dark:bg-gray-700/30 lg:col-span-1">
                ...
                <div className="mt-2 space-y-1">
                  {reservationDetailLines.map((line) => (
                    <div key={`guest-step-summary-${line.roomTypeId}`} className="flex items-start justify-between gap-3 text-sm text-gray-600 dark:text-gray-300">
                      <p>
                        {line.quantity} {line.roomTypeName} room{line.quantity > 1 ? 's' : ''} x {line.nights} night{line.nights > 1 ? 's' : ''}{line.seniorCount > 0 ? ` (${line.seniorCount} senior${line.seniorCount === 1 ? '' : 's'})` : ''}
                      </p>
