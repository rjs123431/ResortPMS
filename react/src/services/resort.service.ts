...
export const resortService = {
  ...
  getAvailableRoomsForChange: async (requestId: string) => {
    const response = await api.get<ApiResponse<AvailableRoomForChangeDto[]>>('/api/services/app/RoomChange/GetAvailableRoomsForChange', {
      params: { requestId },
    });
    return response.data.result;
  },
  ...
  getRoomChangeRequestsByStay: async (stayId: string) => {
    const response = await api.get<ApiResponse<RoomChangeRequestListDto[]>>('/api/services/app/RoomChange/GetRequestsByStay', {
      params: { stayId },
    });
    return response.data.result;
  },
